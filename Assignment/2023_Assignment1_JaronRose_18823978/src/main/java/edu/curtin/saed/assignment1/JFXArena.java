...
/*
 * Jaron Rose 18823978
 * Provided JFXArena class to use for assignment:
 * Minor changes / additions were made to display images
 * on the board
 */
...
public class JFXArena extends Pane
{
    ...
    private static final String BROKEN_IMAGE = "181479.png";
    ...
    public  List<Prop> boardProps = new ArrayList<>();
    ...
    public JFXArena()
    {
        ...
        try(InputStream tower = getClass().getClassLoader().getResourceAsStream(CITADEL_IMAGE))
        {
            if(tower == null)
            {
                throw new AssertionError("Cannot find image file " + CITADEL_IMAGE);
            }
            citadel = new Image(tower);
        }
        catch(IOException e)
        {
        ...
        try(InputStream wall = getClass().getClassLoader().getResourceAsStream(WALL_IMAGE))
        {
            if(wall == null)
            {
                throw new AssertionError("Cannot find image file " + WALL_IMAGE);
            }
            wallPic = new Image(wall);
        }
        catch(IOException e)
        {
        ...
        try(InputStream dmgWall = getClass().getClassLoader().getResourceAsStream(BROKEN_IMAGE))
        {
            if(dmgWall == null)
            {
                throw new AssertionError("Cannot find image file " + BROKEN_IMAGE);
            }
            damagedWall = new Image(dmgWall);
        }
        catch(IOException e)
        {
            throw new AssertionError("Cannot load image file " + BROKEN_IMAGE, e);
        }  
        canvas = new Canvas();
    ...
    public void setCitadellPosition()
    {
        game.createCitadel();
        requestLayout();
    }
    ...
    public void layoutChildren()
    {
        ...
        for(Prop prop : boardProps)
        {
            // Draw Citadel
            if(prop instanceof Citadel)
            {
                drawImage(gfx, citadel, prop.getX(), prop.getY());
            }
            // Draw Robot
            else if(prop instanceof Robot)
            {
                drawImage(gfx, robot1, prop.getX(), prop.getY());
                drawLabel(gfx, "Robot " + prop.getID(), prop.getX(), prop.getY());
            }
            // Draw Wall
            else if(prop instanceof Wall)
            {
                Wall wall = (Wall)prop;
                if(wall.getHealth() == 2)
                {
                    drawImage(gfx, wallPic, wall.getX(), wall.getY());
                }    
                else if(wall.getHealth() == 1)
                {
                    drawImage(gfx, damagedWall, wall.getX(), wall.getY());
                }                                
            }   
        }      
    }
    ...
    private void drawImage(GraphicsContext gfx, Image image, double gridX, double gridY)
    {
        ...
        double displayedPixelWidth;
        double displayedPixelHeight;
